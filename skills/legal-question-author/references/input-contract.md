...
Damophus 题库的“导出个性化出题资料包”是首选输入。它是独立于作答记录归档的 JSON，包含：

- `questions`: 当前题目的完整题干、选项、答案、解析，以及对应的 `catalog` 静态索引元数据；
- `groups`: 题组材料与子题 ID 的关系；
- `topics`: 题库中的考点树；
- `attempts`: 有效作答事件（含 `mastery_rating`、正确性、时间和用时）；
- `aggregates`: 按题目聚合的次数、错题、Again/Hard 连续状态和最近作答时间；
- `bookmarks`: 题目收藏的标签和备注；
- `source_documents`: 题目来源文档的标题、路径和更新时间。

现有的 `damophus-attempts-*.json` 仍然只表示作答事件，不能单独恢复题干、选项、解析或考点；它可以作为资料包之外的补充记录，但不能替代资料包。
