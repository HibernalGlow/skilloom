...
- **Row anchors.** Every knowledge-bearing row or card carries its own compact semantic icon beside the label — an icon chip with the pictogram on the node's tone color, a role glyph, or a document mark that participates in the row's entrance. One icon on the panel title does not anchor the rows beneath it: an icon-titled panel whose body stays a multi-line prose paragraph is still a text wall, so segment the body into icon-anchored rows or chips and give each fact its own glyph. Declare the row-anchor components in the scene's `tokens` so audits pin them.
