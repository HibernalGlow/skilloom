...
- Treat Remotion page-still QA as a blocking iterative loop, not a one-time command:
  ...
  1. After creating or changing one animation, choose exactly one capture mode for its review. Run `pnpm animation:pages <animation-id>` for stable frames. When sustained emphasis is present or changed, run `pnpm animation:pages <animation-id> --motion` instead so its one contact sheet contains all checkpoint frames for all scenes; do not routinely inspect both runs.
  ...
  20. Choose `pnpm animation:pages <animation-id> --motion` as the animation's single capture mode whenever persistent emphasis is added or changed. Inspect its one contact sheet containing the three checkpoint frames for every scene: the teaching layout must remain stable and readable, while at least one authored focal cue changes phase, position, or progress where sustained emphasis is intended. If motion QA becomes necessary only after a stable-sheet review, the motion sheet supersedes that earlier review; do not load both sheets as separate evidence.
  ...
  20b. Inspect stable-frame content density. Reject paragraph-dependent scenes, competing reading paths, small teaching islands surrounded by purposeless empty canvas, and decoration added only to simulate density. Require the visible actors, conditions, actions, relations, results, and exceptions to justify the occupied space.
