...
- When a new direction is introduced, confirm `visual-direction.json` marks its original node with `catalog.status: "featured"`, `catalog.source: "original-node"`, and a valid `/inkloom/.../` route. Confirm the `/demo/` root previews the original composition and opens that existing MDX route; reject a duplicate demo composition or carrier.
...
- Confirm every changed composition reports `DURATION_FRAMES / FPS <= 20` unless the user explicitly approved a longer runtime. Check the actual player duration rather than relying on a source-duration constant, and reject a nominally short composition whose playback-rate conversion makes the delivered runtime longer.
...
- Treat Remotion page-still QA as a blocking iterative loop, not a one-time command:
  ...
  20a. Verify the complete player duration is no more than 20 seconds and inspect motion at that final timing. Every essential label and conclusion must still receive a readable hold; reject speed-ups that technically meet the duration limit but make the reasoning difficult to follow.
  ...
  22. For every new featured direction, open `/inkloom/demo/` and verify its preview uses the original composition, its label matches the manifest, and its link opens `catalog.route` rather than a demo-only clone.
