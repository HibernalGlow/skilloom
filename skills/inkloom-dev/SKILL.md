...
9. After page-still QA passes, run `pnpm animation:publish-stills <animation-id>`. It selects every scene's stable final frame, encodes versioned WebP quality 60 assets, writes the manifest, and updates the carrier's rendered image block. Inspect its output; do not manually copy, encode, rename, or link final-frame assets.
...
- Do not leave approved final-frame screenshots only in `.artifacts`, hide them behind links or disclosure widgets, or reference a mutable unversioned filename from the carrier. Use `pnpm animation:publish-stills <animation-id>` to publish and directly render one versioned WebP quality 60 final frame per semantic scene beside the MD/MDX file; do not publish PNG final-frame assets or recreate the command's output manually.
