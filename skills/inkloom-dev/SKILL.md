...
9. After page-still QA passes, follow [animated-webp.md](references/animated-webp.md) and run `pnpm animation:publish-webp <animation-id>`. Publish one q45, once-playing animated WebP per semantic scene under `public/animation-webp/<animation-id>/`; never restore the retired static final-frame publishing flow.
10. Validate the responsive video/WebP switcher, replay and loop behavior, copy actions, persisted media preference, public assets, and production build only after page-still QA and animated-WebP publication pass. Commit and push the animation sources, player components, metadata, MDX carriers, animated WebPs, manifests, and SiYuan controller script to InkLoom. Verify the production page and direct WebP URLs before reporting them as published.
...
12. Validate the changed pages and any SiYuan embeds with [validation.md](references/validation.md). Report any unverified route, animation page, animated-WebP asset, deployment URL, or SiYuan block insertion.
...
- Animated WebP publication: `pnpm animation:publish-webp <animation-id>` for changed animations and `pnpm animation:publish-webp` for a completed batch.
...
- Do not restore `animation:publish-stills`, versioned q60 final-frame assets, or `## 场景终帧` blocks. Page-still PNGs remain QA-only under `.artifacts`; public companion media is the per-scene animated WebP set defined in [animated-webp.md](references/animated-webp.md).
- Do not add `## 动态...` description titles (such as `## 动态判断路径` or `## 动态选择路径`) or `## 场景终帧` sections to an MDX carrier. The carrier must contain only the animation component import and frontmatter; the shared player owns video/WebP presentation and copy actions.
...
- [animated-webp.md](references/animated-webp.md): required per-scene q45 animated-WebP publication, website controls, SiYuan controller, and asset verification.
