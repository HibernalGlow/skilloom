...
8. Complete the mandatory iterative page-still QA in [validation.md](references/validation.md) before calling any animation finished. Review exactly one generated contact sheet per animation, with all semantic scenes arranged in that sheet. Use `pnpm animation:pages <animation-id>` for stable-frame QA, or use `pnpm animation:pages <animation-id> --motion` as the single review run when sustained emphasis needs motion checkpoints. Do not open every 1920x1080 `page-*.png`; keep those files only as crop sources. If one contact-sheet tile is suspicious, crop and enlarge only the questionable region from its source PNG, inspect that crop, fix the source, and regenerate the animation's one contact sheet.
