...
- Animation duration: keep the complete composition at or below 20 seconds by default, measured as `DURATION_FRAMES / FPS`. Re-stage the explanation and use the canvas more efficiently when needed; preserve key legal wording and trim only redundant phrasing or non-semantic metadata. Do not use an unreadably high playback rate merely to pass the limit.
...
- Do not let a default explainer exceed 20 seconds of complete composition time (`DURATION_FRAMES / FPS`). If the legal point cannot remain legible within the limit, narrow the learning objective or split the explanation instead of extending the runtime or speeding motion past comfortable reading.
