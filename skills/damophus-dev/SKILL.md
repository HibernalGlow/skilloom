...
- **UI component policy**: For every Svelte UI change, inspect the repository's installed component stack before writing markup. Prefer existing `shadcn-svelte` components first; when a needed primitive is not yet present, acquire the existing component instead of falling back to an older business widget. Use the project's package manager and shadcn-svelte generator/registry when available; otherwise install a maintained compatible Svelte primitive/library, prioritizing `bits-ui`, `melt-ui`, `lucide-svelte`, and other libraries that preserve the shadcn styling and accessibility model. Commit generated component files, dependency entries, and the lockfile together. Compose these primitives into domain-specific views, but do not hand-roll foundational controls or interaction primitives: no custom buttons, inputs, selects, comboboxes, dialogs, popovers, menus, tabs, tooltips, switches, dropdowns, toasts, command palettes, focus traps, keyboard navigation, or modal behavior implemented from raw HTML/CSS/events. Do not draw replacement SVG icons when a library icon exists. A bespoke business layout that composes library components is allowed; a pre-existing business component is not a substitute when a suitable maintained library component can be installed. If registry/package discovery genuinely finds no compatible implementation or installation is blocked, report the blocker and get explicit approval before introducing a new primitive.
- **UI decision record**: Before adding a UI dependency or generated component, record the searched shadcn-svelte registry and compatible libraries, the selected component/version, the accessibility/state behavior it supplies, and why any existing business widget was not reused. Dependency installation is the default implementation path, not an approval gate; explicit approval is required only for a new hand-written foundational primitive. A component change is incomplete if its diff contains a new foundational primitive without that record.
