...
1. Select **ordinary mode** when the request is note organization, summarization, formatting, or repair and does not explicitly ask for flashcards. Read [ordinary-mode.md](references/ordinary-mode.md). Completion criterion: the note preserves its source contract, records only review candidates, and contains no formal `custom-dm-*` card IAL or runtime SRS field.
2. Select **dedicated-card mode** only when the user explicitly asks to make, generate, draft, export, or review flashcards/cards. Read [dedicated-mode.md](references/dedicated-mode.md) and then [protocol-contract.md](references/protocol-contract.md). Choose `basic` for focused Q/A, `cloze` for a short deletion, and `mnemonic` for a source-grounded口诀 or other non-Q/A memory cue. Completion criterion: every accepted card has a validated root container, stable identity, source grounding, and topic reference, or generation stops with a named gap.
...
- Draft candidates before acceptance. Apply the deterministic checker in `scripts/validate_flashcard.py` to the dedicated draft; then report candidate, accepted, rejected counts and rejection reasons. Completion criterion: the report reconciles to the number of candidate records.
