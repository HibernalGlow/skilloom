...
2. Build the source-fact ledger from [card-design.md](card-design.md) before drafting. Include each independently testable rule, fixed list, distinction, exception, trap, date, case-derived new rule/application, and source-written mnemonic, including facts found only in examples or answer explanations. Do not make cards from an original case narrative, party names, chronology, or question stem; record `case-narrative` and reject it unless the case contributes a reusable rule, exception, distinction, application path, or trap. Completion criterion: every fact has one source locator, recall axis, provider, tags, and disposition slot; every case range has either a reusable extracted proposition or an explicit `case-narrative` rejection; nothing is silently omitted because it appeared late in the source.
...
4. Draft candidates by applying the recall-unit gate in [card-design.md](card-design.md). For every non-simple back or source range containing ordered steps, nested rules, a substantive Callout, true comparison table, or recall-sized diagram, read [answer-structure.md](answer-structure.md) and inherit or select the matching carrier. Run the clause split before rendering: long or multi-clause answer lines become a governing parent with source-shaped children unless the proposition is genuinely inseparable. For medium or complex relations involving process, branches, role mapping, comparison, or many-to-one structure, generate or inherit at least one Mermaid carrier unless a source-shaped table or image is the materially better primary carrier; a plain list-only answer is a warning requiring disposition. When the user requests a question-side diagram or a complex process, branch, role relation, or many-to-one mapping materially benefits from a retrieval skeleton, read [question-side-mermaid.md](question-side-mermaid.md); keep that branch limited to `basic/list`. Split definition, composition, status, policy, condition, procedure, exception, legal effect, distinction, and historical source when independently gradable. Completion criterion: every candidate has one expected-answer statement, one semantic kind, one stable parent/variant identity, and a source-grounded answer structure that exposes rather than flattens its legal relation; any front visual passes its separate boundary and leakage gate, and every eligible complex relation has a Mermaid, inherited image, or documented carrier exception.
