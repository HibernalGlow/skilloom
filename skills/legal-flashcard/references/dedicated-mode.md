...
4. Draft candidates by applying the recall-unit gate in [card-design.md](card-design.md). For every non-simple back or source range containing ordered steps, nested rules, a substantive Callout, true comparison table, or recall-sized diagram, read [answer-structure.md](answer-structure.md) and inherit or select the matching carrier. Split definition, composition, status, policy, condition, procedure, exception, legal effect, distinction, and historical source when independently gradable. Completion criterion: every candidate has one expected-answer statement, one semantic kind, one stable parent/variant identity, and a source-grounded answer structure that exposes rather than flattens its legal relation.
...
- Basic cards use one direct question root `- ...？` and an unlabeled semantic back structure. Follow [answer-structure.md](answer-structure.md) for nested unordered/ordered lists, tables, Mermaid, and source-container inheritance. Attach the card IAL to the complete root block, outside any table row or code fence.
