...
- `W128`: the source carries mnemonic material — a 口诀 label or an implicit mnemonic (inline-code compact sequence, memory Callout, 谐音/缩写/取字 mapping) — but the deck has no `mnemonic` card; turn it into a mnemonic card with a highlighted cue and decoded segments. A wrong-answer Case card may instead carry it on the back in a dedicated `> [!MNEMONIC]` block (or a memory `TIP`/`NOTE`/`IMPORTANT` titled with 记忆/口诀/谐音/取字/首字) that quotes the 06-口诀 or provider sentence verbatim and, when that sentence is too long, adds a `==取字简写≤7字== → 短展开` line; the block is the card's mnemonic region and is exempt from `E029`, `E095`, and `E086`. Emit no block when the source has nothing to carry — an invented cue is a defect, not a pass.
