...
- `missing-tag`: no source-grounded `#法考/...#` knowledge tag can be confirmed for an accepted card.
...
The checker must fail on invalid attribute names, multiline or malformed IALs, schema values, card IDs, renderer values, duplicate IDs, detached roots, code-fence IALs, malformed or over-reused note-topic IDs, unresolved broad-topic fallback, missing `#法考/...#` knowledge tags, invalid `#闪卡/优先级/P1#`-`P4` tags, missing MarkNote anchors, oversized answer items, excessive answer counts, report mismatches, runtime-field leakage, and missing required fields. A passing result is necessary but not sufficient for legal accuracy.
