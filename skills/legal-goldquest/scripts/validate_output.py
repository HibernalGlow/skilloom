...
GATE_EXPLAIN: dict[str, str] = {
    ...
    "E820": "考点 tag 缺失：GoldQuest 每道题的 H5 标题行尾、MarkNote 的 note-topic provider 块必须带 #法考/科目/[专题/]考点#。MarkNote 有标题→标题行尾追加；只有 **考点：x** 锚点→IAL 行后独立 tag 段（锚点行尾禁写）。词表见 references/kaodian-tags.md，按考点名逐字复用完整路径。",
    "E821": "考点 tag 未闭合或含空格：tag 是严格的 #法考/…# 定界写法，内部不许有空格、# 或其他定界符。",
    "E822": "考点 tag 位置非法：只允许 GoldQuest 题目 H5 行、MarkNote provider 标题行或其锚点块后的独立 tag 段；禁止进代码围栏、题干、解析或 IAL 行。",
