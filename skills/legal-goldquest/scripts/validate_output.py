...
def validate_kaodian_tags(text: str, profile: str):
    from legal_kaodian_tag_gate import validate_kaodian_tags as _gate

    return _gate(text, profile)
...
def validate_text(
    ...
    findings.extend(validate_kaodian_tags(text, profile))
    if profile == "legal-goldquest":
...
GATE_EXPLAIN: dict[str, str] = {
    ...
    "E820": "考点 tag 缺失：GoldQuest 每道题「正确答案」行尾、MarkNote 的 note-topic provider 块必须带 #法考/科目/[专题/]考点#。GoldQuest 练习视图只渲染题干/选项/解析，tag 放解析区首行（正确答案行尾）才能在复盘时看到且不剧选题面；MarkNote 有标题→标题行尾追加，只有 **考点：x** 锚点→IAL 行后独立 tag 段。词表见 references/kaodian-tags.md，按考点名逐字复用完整路径。",
    "E821": "考点 tag 未闭合或含空格：tag 是严格的 #法考/…# 定界写法，内部不许有空格、# 或其他定界符。",
    "E822": "考点 tag 位置非法：只允许 GoldQuest「正确答案」行尾、MarkNote provider 标题行或其锚点块后的独立 tag 段；禁止进代码围栏、标题行、题干、选项、解析正文或 IAL 行。",
    "W823": "tag 不在闪卡词汇表快照：新考点先向用户确认路径，再把完整路径加进 references/kaodian-tags.md 后重验；禁止给已有考点造同义路径。",
    "W517": "Emoji 密度应随列表嵌套深度递增——有子列表的顶级项必须至少一个 emoji，且每个子列表项也须 ≥1 个 emoji（目标：子项 emoji 数 >= 父项 emoji 数），确保大体量嵌套列表获得更多语义视觉锚点。",
}
