...
- **Mermaid 的语义触发**: use it for procedures, sequences, branches, comparisons, or subject relationships. Choose the diagram that explains the actual material; never generate a fixed list of headings merely to satisfy the rule. Mermaid must contain a semantic style directive: use `%%{init: ...}%%` with `themeVariables`, `classDef`, or targeted `style` rules. Keep node labels short, quote labels containing punctuation, use `<br/>` only for deliberate soft breaks, and choose a direction (`TD`/`LR`) that matches the reading flow.
