...
8. Apply the active-coverage, density, boundary, and callout rules in [the detailed guide](references/note-guide-original.md). Keep ordinary prose lines within 42 visible characters and split longer reasoning at semantic boundaries into nested lists. For medium or complex material, use a rich presentation selected from the content: at least four auxiliary style families (highlight, italic, strikethrough, inline code, underline), at least four structural families (nested lists, Callout, subheadings, tables, Mermaid, dividers), and at least three short background-color anchors. Use Mermaid when the content has a sequence, branch, procedure, comparison, or relationship; do not force a fixed section template.
9. Audit tables by information shape. Keep concise comparison cells in tables, and use `<br />` for a few short numbered items when that improves scanning. When a cell contains a large list, normally four or more independently reviewable items, or three long items totaling about 100 visible characters, move the category and every item into a real nested Markdown list; remove the generated table, or leave an existing source table unchanged and place the usable list beside it. Keep one inseparable prerequisite-to-procedure-to-consequence rule in one list item. Apply the full active-color, line-length, hierarchy, and rich-visual contract to every converted list.
10. Run `python -X utf8 scripts/validate_output.py <output.md> --strict --require-topic-ial` and `python -X utf8 scripts/audit_question_groups.py <output.md>`. When the original material is available as a file or snapshot, run both source-aware gates: add `--source <source.md> --require-source` to `validate_output.py`, and run `python -X utf8 scripts/audit_question_groups.py <output.md> --source <source.md>` to prove that question and subquestion identifiers were not changed. Treat `E404`–`E410` as a mandatory merge-grid audit before delivery.
...
- **Rich mode**: when a block is medium or complex, 至少四类辅助样式 from `==高亮==`, `_斜体_`, `~~删除线~~`, inline code, and `<u>下划线</u>`; 至少四类结构载体 from nested lists, Callout, subheadings, tables, Mermaid, and dividers; 至少三个短背景色锚点。
- **Mermaid 的语义触发**: use it for procedures, sequences, branches, comparisons, or subject relationships. Choose the diagram that explains the actual material; never generate a fixed list of headings merely to satisfy the rule. When a diagram is needed, invoke the installed `beautiful-mermaid` skill to choose the theme, palette, node styles, labels, and direction; keep node labels short and make the visual hierarchy match the legal relationships.
