...
3. Format question-and-answer groups before styling body content. A single fenced `md` question block should contain only one to three closely related questions; when a source block contains more, split it at a meaningful topic or decision boundary. Put the matching `**回答与解析：**` section immediately after each block, keep the original question numbers, and never place answers for one block after a later question block. Treat multi-part questions such as `(1)`–`(3)` as one question with an indented sublist when they share one fact pattern.
3a. Separate source content from added summaries or study prompts.
...
For a question set with many assertions, do not use one large fenced block for all questions. Group one to three questions that test the same rule or decision boundary in a single `> ```md` block, then close it and place that group's `**回答与解析：**` directly below. Repeat this pattern for the next group. The fence is for the learner's question prompt only; answers and explanations remain ordinary Markdown inside the surrounding quote block.
