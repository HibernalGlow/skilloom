...
- **Mermaid 的语义触发**: use it for procedures, sequences, branches, comparisons, or subject relationships. Choose the diagram that explains the actual material; never generate a fixed list of headings merely to satisfy the rule. When a diagram is needed, **先调用已安装的 `beautiful-mermaid` 技能**，读取其工作流并渲染 SVG/PNG 预览；检查主题、配色、节点样式、标签长度、方向和连线后再交付，不能只手写一个原生 Mermaid 代码块。输出目标支持图片资源时，在 Mermaid 源码旁引用渲染后的 SVG/PNG；需要保持可编辑时保留 Mermaid 源码，但仍不得跳过渲染预览。
